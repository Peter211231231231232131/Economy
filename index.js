...
client.on('interactionCreate', async (interaction) => {
    ...
    if (commandName === 'link') {
        ...
        if (existingLink) return interaction.editReply({ content: `Your Discord account is already linked to **${existingLink._id}**!` });
    ...
    let result, amount, choice, itemName;
    switch (commandName) {
        ...
        case 'market': const listings = await marketCollection.find().toArray(); if(listings.length === 0) return interaction.editReply({content: 'The market is empty.'}); let marketMessage = listings.map(l => `(ID: ${l._id.toString().slice(-6)}) **${l.quantity}x** ${ITEMS[l.itemId].name} @ **${l.price}** ${CURRENCY_NAME} ea. by *${l.sellerName}*`).join('\n'); await interaction.editReply({content: `**Market Listings:**\n${marketMessage}`}); break;
        case 'marketsell': itemName = options.getString('item_name'); const quantity = options.getInteger('quantity'); const price = options.getNumber('price'); const itemId = getItemIdByName(itemName); if(!itemId) return interaction.editReply({content: 'Invalid item name.'}); if(quantity <= 0 || price <= 0) return interaction.editReply({content: 'Quantity and price must be positive.'}); if((account.inventory[itemId] || 0) < quantity) return interaction.editReply({content: 'You do not have enough of that item to sell.'}); await modifyInventory(account._id, itemId, -quantity); const listing = await marketCollection.insertOne({ sellerId: account._id, sellerName: account._id, itemId, quantity, price }); await interaction.editReply({content: `You listed ${quantity}x ${ITEMS[itemId].name} for sale. Listing ID: ${listing.insertedId.toString().slice(-6)}`}); break;
        case 'marketbuy': const listingId = options.getString('listing_id'); let listingToBuy; try { listingToBuy = await marketCollection.findOne({ _id: new ObjectId(listingId) });} catch(e) { /* Handle invalid IDs */ } if(!listingToBuy) return interaction.editReply({content: 'Invalid listing ID.'}); if(listingToBuy.sellerId === account._id) return interaction.editReply({content: "You can't buy your own listing."}); const totalCost = listingToBuy.quantity * listingToBuy.price; if(account.balance < totalCost) return interaction.editReply({content: `You can't afford this. It costs ${totalCost} ${CURRENCY_NAME}.`}); await updateAccount(account._id, { balance: account.balance - totalCost }); await modifyInventory(account._id, listingToBuy.itemId, listingToBuy.quantity); const sellerAccount = await getAccount(listingToBuy.sellerId); if(sellerAccount) await updateAccount(sellerAccount._id, { balance: sellerAccount.balance + (totalCost * (1 - MARKET_TAX_RATE)) }); await marketCollection.deleteOne({_id: new ObjectId(listingId) }); await interaction.editReply({content: `You bought ${listingToBuy.quantity}x ${ITEMS[listingToBuy.itemId].name}!`}); break;
        case 'marketcancel': const cancelId = options.getString('listing_id'); let listingToCancel; try { listingToCancel = await marketCollection.findOne({ _id: new ObjectId(cancelId) });} catch(e) { /* Handle invalid IDs */ } if(!listingToCancel || listingToCancel.sellerId !== account._id) return interaction.editReply({content: 'This is not your listing or it does not exist.'}); await modifyInventory(account._id, listingToCancel.itemId, listingToCancel.quantity); await marketCollection.deleteOne({_id: new ObjectId(cancelId)}); await interaction.editReply({content: `You cancelled your listing for ${listingToCancel.quantity}x ${ITEMS[listingToCancel.itemId].name}. The items have been returned.`}); break;
    }
});
...
app.post('/command', async (req, res) => {
    ...
    // ... Verification and account creation logic ...
    // ... (This section is now much larger to handle all commands)
