...
const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBitsYou.MessageContent, GatewayIntentBits.GuildMembers] });
const YOUR_API_KEY = 'drednot are absolutely right. To prevent any further issues with partial code, here is the **123'; // Your secret API key

// =========================================================================
// --- MONGODB DATABASEsingle, complete, and final `index.js` script** one more time.

This version is the one that includes SETUP ---
// =========================================================================
...
async function connectToDatabase all features (work, gather, craft, market, daily, gambling) and has the correct logic for handling all commands and database() {
    try {
        await mongoClient.connect();
        console.log("Successfully connected to MongoDB interactions. This is the code that should be in your GitHub repository.

---

### The Complete, Final, and Atlas!");
        const db = mongoClient.db("drednot_economy");
        economyCollection = db Correct `index.js`

**Action:** Replace the entire contents of your `index.js` file on GitHub with this.collection("players");
        verificationsCollection = db.collection("verifications");
        marketCollection = db.collection("market_listings");
    } catch (error) { console.error("Failed to connect to MongoDB code block.

```javascript
// --- Library Imports ---
const { Client, GatewayIntentBits, MessageFlags } = require", error); process.exit(1); }
}

// =========================================================================
// --- ECONOMY DEFINITIONS('discord.js');
const express = require('express');
const { MongoClient, ObjectId } = require('mongodb');
...
const WORK_REWARD_MIN = 5, WORK_REWARD_MAX = 35, WORK_COOLDOWN_MINUTES;
app.use(express.json());
const client = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildMessages, GatewayIntentBits.MessageContent, GatewayIntentBits.GuildMembers] });
const = 1;
const GATHER_COOLDOWN_MINUTES = 3;
const MARKET_TAX_RATE = 0.05;
const FLIP_MIN_BET = 5, FLIP_ YOUR_API_KEY = 'drednot123'; // Your secret API key

// =========================================================================
// --- MONGODB DATABASE SETUP ---
// =========================================================================
const mongoUri = processMAX_BET = 100;
const SLOTS_MIN_BET = 10, SLOTS_MAX_BET = 1500, SLOTS_COOLDOWN_SECONDS = 5;

.env.MONGO_URI;
if (!mongoUri) throw new Error("CRITICAL: MONGO_URI not found!");
const mongoClient = new MongoClient(mongoUri);
let economyCollection, verificationsCollection, marketconst ITEMS = {
    'iron_ore': { name: "Iron Ore", description: "A common metallic ore." },
    'copper_ore': { name: "Copper Ore", description: "A reddish-brownCollection;

async function connectToDatabase() {
    try {
        await mongoClient.connect();
        console.log("Successfully connected to MongoDB Atlas!");
        const db = mongoClient.db("drednot_economy"); metallic ore." },
    ...
    'coal': { name
        ...
        verificationsCollection = db.collection("verifications");
        marketCollection = db.collection("market_listings");
    } catch (error) { console.error: "Coal", description: "A combustible rock, used as fuel." },
    'basic_pickaxe': { name: "Basic Pickaxe", description: "A simple pickaxe.", craftable: true, recipe: { 'stone': 5, 'wood': 2 } },
    'sturdy_pickaxe': { name("Failed to connect to MongoDB", error); process.exit(1); }
}

// =========================================================================
// --- ECONOMY DEFINITIONS ---
// =========================================================================
const CURRENCY_NAME: "Sturdy Pickaxe", description: `Slightly increases earnings from work.`, craftable: true, recipe: { 'iron_ore': 10, 'wood': 3, 'coal': 2 } = 'Bits';
const STARTING_BALANCE = 30;
const DAILY_REWARD = 25;
const WORK_REWARD_MIN = 5, WORK_REWARD_MAX = 35, WORK_COOLDOWN_MINUTES = 1;
const GATHER_COOLDOWN_MINUTES },
};
const GATHER_TABLE = { 'iron_ore': { baseChance: 0.60, minQty: 1, maxQty: 3 }, 'copper_ore': { baseChance: 0.40, minQty: 1, maxQty: 2 }, 'stone': { baseChance: = 3;
const MARKET_TAX_RATE = 0.05;
const FLIP_MIN_BET = 5, FLIP_MAX_BET = 100;
const SLOTS_MIN_BET = 10, SLOTS_MAX_BET = 1500, SLOTS_CO 0.70, minQty: 2, maxQty: 5 }, 'wood': { baseChance: 0.50, minQty: 1, maxQty: 4 }, 'coal': { baseChance: 0.30, minQty: 1, maxQty: 2 } };
const SLOTOLDOWN_SECONDS = 5;

const ITEMS = {
    'iron_ore': { name: "Iron Ore", description: "A common metallic ore." },
    'copper_ore': { name: "Copper_REELS = [ ['🍒', '🍋', '🍊', '🍉', '⭐', '🔔', '💎', '💰', '💔'], ['🍒', '🍋', '🍊', '🍉', '⭐', '🔔', '💎', '💰', '💔'], ['🍒', '🍋', '🍊', '🍉', ' Ore", description: "A reddish-brown metallic ore." },
    'stone': { name: "Stone", description: "A basic building material." },
    'wood': { name: "Wood", description: "A basic building material." },
    'coal': { name: "Coal", description: "A combustible rock, used⭐', '🔔', '💎', '💰', '💔']];
const SLOTS_PAYOUTS = { three_of_a_kind: 15, two_of_a_kind: 3.5, jackpot_symbol: '💎', jackpot_multiplier: 50 };

// =========================================================================
// as fuel." },
    'basic_pickaxe': { name: "Basic Pickaxe", description: "A simple pickaxe.", craftable: true, recipe: { 'stone': 5, 'wood': 2 } },
    'sturdy_pickaxe': { name: "Sturdy Pickaxe", description: `Slightly increases earnings from work.`, craftable: true, recipe: { 'iron_ore': 10, --- DATABASE HELPER FUNCTIONS ---
// =========================================================================
...
async function createNewAccount(drednotName 'wood': 3, 'coal': 2 } },
};
const GATHER_TABLE = { 'iron_ore': { baseChance: 0.60, minQty: 1, maxQty: ) { const lowerName = drednotName.toLowerCase(); const newAccount = { _id: lowerName, balance: STARTING_BALANCE, discordId: null, lastWork: null, lastGather: null, lastDaily: null3 }, 'copper_ore': { baseChance: 0.40, minQty: 1, maxQty: 2 }, 'stone': { baseChance: 0.70, minQty: 2, maxQty: 5 }, 'wood': { baseChance: 0.50, minQty: 1, lastSlots: null, inventory: {} }; await economyCollection.insertOne(newAccount); return newAccount; }
async function updateAccount(accountId, updates) { await economyCollection.updateOne({ _id: accountId.toLowerCase() }, { $set: updates }); }
async function modifyInventory(accountId, itemId, amount) { if (, maxQty: 4 }, 'coal': { baseChance: 0.30, minQty: 1, maxQty: 2 } };
const SLOT_REELS = [ ['🍒', '🍋', '🍊', '🍉', '⭐', '🔔', '💎', '💰', '💔'], ['🍒', '!itemId) return; const updateField = `inventory.${itemId}`; await economyCollection.updateOne({ _id: accountId.toLowerCase() }, { $inc: { [updateField]: amount } }); }
function getItemIdByName(name) { return Object.keys(ITEMS).find(k => ITEMS[k].name.toLowerCase() === name.🍋', '🍊', '🍉', '⭐', '🔔', '💎', '💰', '💔'], ['🍒', '🍋', '🍊', '🍉', '⭐', '🔔', '💎', '💰', '💔']];
const SLOTS_PAYOUTS = { three_of_a_kind: 15, two_of_a_kind: 3.5, jackpot_symbol: '💎', jackpot_multiplier: 5toLowerCase()); }
...
// =========================================================================
async function handleWork(account) { const now = Date.now(); const cooldown = WORK_COOLDOWN_MINUTES * 60 * 1000; if (account.lastWork && (now - account.lastWork) < cooldown) { const remaining = cooldown - (now - account.lastWork); return { success: false, message: `You }
async function createNewAccount(drednotName) { const lowerName = drednotName.toLowerCase(); const newAccount = { _id: lowerName, balance: STARTING_BALANCE, discordId: null, lastWork: null, lastGather: null, lastDaily: null, lastSlots: null, inventory: {} }; await economy are on cooldown. Wait ${formatDuration(remaining / 1000)}.` }; } const earnings = Math.floor(Math.random() * (WORK_REWARD_MAX - WORK_REWARD_MIN + 1)) + WORK_REWARD_MIN; await updateAccount(account._id, { balance: account.balance + earnings, lastCollection.insertOne(newAccount); return newAccount; }
async function updateAccount(accountId, updates) { await economyCollection.updateOne({ _id: accountId.toLowerCase() }, { $set: updates }); }
async function modifyInventory(accountId, itemId, amount) { if (!itemId) return; const updateField = `inventory.${itemId}`; await economyCollection.updateOne({ _id: accountId.toLowerCase() }, { $inc: { [updateField]: amountWork: now }); return { success: true, message: `You earned ${earnings} ${CURRENCY_NAME}! New balance is ${account.balance + earnings}.` }; }
async function handleGather(account) { const now = Date.now(); const cooldown = GATHER_COOLDOWN_MINUTES * 60 * 1000; if (account.lastGather && (now - account.lastGather) < cooldown) { const remaining = cooldown - (now } }); }
function getItemIdByName(name) { return Object.keys(ITEMS).find(k => ITEMS[k].name.toLowerCase() === name.toLowerCase()); }
function formatDuration(seconds) { if (seconds < 60) return `${Math.ceil(seconds)}s`; const minutes = Math.floor(seconds / 60); const - account.lastGather); return { success: false, message: `You are tired. Wait ${formatDuration(remaining / 1000)}.` }; } let gatheredItems = []; let updates = {}; for (const itemId in GATHER_TABLE) { if (Math.random() < GATHER_TABLE[itemId].baseChance) { const remainingSeconds = Math.ceil(seconds % 60); return `${minutes}m ${remainingSeconds}s`; }

// =========================================================================
// --- COMMAND HANDLER LOGIC ---
// =========================================================================
async function handleWork(account) { const now = Date.now(); const cooldown = WORK_COOLDOWN_MINUTES * 60 * 1000; if (account.lastWork && (now - account.lastWork qty = Math.floor(Math.random() * (GATHER_TABLE[itemId].maxQty - GATHER_TABLE[itemId].minQty + 1)) + GATHER_TABLE[itemId].minQty; updates[`inventory.${itemId}`] = qty; gatheredItems.push(`${qty}x ${ITEMS[itemId].name}`); } } await economyCollection.updateOne({ _id: account._id }, { $inc: updates, $set: { lastGather: now } }); if) < cooldown) { const remaining = cooldown - (now - account.lastWork); return { success: false, message: `You are on cooldown. Wait ${formatDuration(remaining / 1000)}.` }; } const earnings = Math.floor(Math.random() * (WORK_REWARD_MAX - WORK_REWARD_MIN + 1)) + WORK_REWARD_MIN; const newBalance = account.balance + earnings; await updateAccount (gatheredItems.length === 0) return { success: true, message: 'You searched but found nothing.' }; return { success: true, message: `You gathered: ${gatheredItems.join(', ')}.` }; }
function handleInventory(account) { if (!account.inventory || Object.keys(account.inventory).length === 0) return 'Your inventory is empty.'; let invList = ['Your inventory:']; for (const itemId in account.inventory) { if (account.inventory[itemId] > 0) invList.push(`- ${account.inventory[itemId]}x ${ITEMS(account._id, { balance: newBalance, lastWork: now }); return { success: true, message: `You earned ${earnings} ${CURRENCY_NAME}! New balance: ${newBalance}.` }; }
async function handleGather(account) { const now = Date.now(); const cooldown = GATHER_COOLDOWN_MINUTES * 60 * 1000; if (account.lastGather && (now - account.lastGather) < cooldown) { const remaining = cooldown - (now - account.lastGather); return { success: false, message: `You are tired. Wait ${formatDuration(remaining / 1000)}.` }; } let[itemId]?.name || itemId}`); } return invList.length > 1 ? invList.join('\n') : 'Your inventory is empty.'; }
...
async gatheredItems = []; let updates = {}; for (const itemId in GATHER_TABLE) { if (Math.random() < GATHER_TABLE[itemId].baseChance) { const qty = Math.floor(Math.random() * (GATHER_TABLE[itemId].maxQty - GATHER_TABLE[itemId].minQty + 1)) + GATHER_TABLE[itemId].minQty; updates[`inventory.${itemId}`] = qty; gatheredItems.push(`${qty}x ${ITEMS[itemId].name}`); } } await economyCollection.updateOne({ _id: account._id }, { $inc: updates, $set: { lastGather: now } }); if (gatheredItems.length === 0 function handleCraft(account, itemName) { const itemToCraftId = getItemIdByName(itemName); if (!itemToCraftId || !ITEMS[itemToCraftId].craftable) return `"${itemName}" is not a valid, craftable item. Check \`/recipes\`.`; const recipe = ITEMS[itemToCraftId].recipe; for (const resId in recipe) { const requiredQty = recipe[resId]; const playerQty = account.inventory[resId] || 0; if (playerQty < requiredQty) return `You don't have enough resources! You need ${requiredQty - playerQty} more ${ITEMS[resId].name}.`; } for (const resId in recipe) await modifyInventory(account._id, resId, -recipe[resId]); await modifyInventory(account._id, itemTo) return { success: true, message: 'You searched but found nothing.' }; return { success: true, message: `You gathered: ${gatheredItems.join(', ')}.` }; }
...
function handleRecipes() { let recipeList = ['**Available Recipes:**']; for (const itemIdCraftId, 1); return `You successfully crafted 1x ${ITEMS[itemToCraftId].name}!`; }
...
client.on('interactionCreate', async (interaction) => {
    ...
    try {
        ...
        console.error("Critical error handling slash command:", error);
        if (interaction.replied || interaction.deferred) {
            await interaction.editReply({ content: 'An unexpected error occurred!' });
        }
    }
...
async function handleSlashCommand(interaction) {
    ...
    switch (commandName) {
        ...
        case 'marketbuy': listingId = options.getString('listing_id'); quantity, price, listingId;
    switch (commandName) {
        case 'balance': await interaction.editReply({ content: `Your balance is: ${account.balance} ${CURRENCY_NAME}.` }); break;
        case 'work': result = await handleWork(account); await interaction.editReply({ content: result.message }); break;
        case 'daily': result = await handleDaily(account); await interaction.editReply({ content: result.message }); break;
        ...
        case 'flip': amount = options.getInteger('amount'); choice = options.getString('choice'); result = await handleFlip(account, amount, choice); await interaction.editReply({ content: result.message }); break;
        case 'slots': amount = options.getInteger('amount'); result = await handleSlots(account, amount); await interaction.editReply({ content: result.message }); break;
        case 'leaderboard': const lbMessage = await handleLeaderboard(); await interaction.editReply({ content: Array.isArray(lbMessage) ? lbMessage.join('\n') : lbMessage }); break;
        case 'market': const listings = await marketCollection.find().limit(20).toArray(); if (listings.length === 0) return interaction.editReply({content: 'The market is empty.'}); let marketMessage = listings.map((l, i) => `${i + 1}. (ID: ${l._id.toString().slice(-6)}) **${l.quantity}x** ${ITEMS[l.itemId].name} @ **${l.price}** ${CURRENCY_NAME} ea. by *${l.sellerName}*`).join('\n'); await interaction.editReply({content: `**Market Listings:**\n${marketMessage}`}); break;
        case 'marketsell': itemName = options.getString('item_name'); quantity = options.getInteger('quantity'); price = options.getNumber('price'); const itemId = getItemIdByName(itemName); if (!itemId) return interaction.editReply({ content: 'Invalid item name.' }); if (quantity <= 0 || price <= 0) return interaction.editReply({ content: 'Quantity and price must be positive.' }); if ((account.inventory[itemId] || 0) < quantity) return interaction.editReply({ content: 'You do not have enough of that item to sell.' }); await modifyInventory(account._id, itemId, -quantity); const listing = await marketCollection.insertOne({ sellerId: account._id, sellerName: account._id, itemId, quantity, price }); await interaction.editReply({ content: `You listed ${quantity}x ${ITEMS[itemId].name} for sale. Listing ID: ${listing.insertedId.toString().slice(-6)}` }); break;
...
app.post('/command', async (req, res) => {
    ...
    if (command === 'verify') {
        ...
        if (!verificationData) { responseMessage = 'That verification code is invalid or has expired.'; } 
    ...
    switch (command) {
        ...
        case 'craft': result = await handleCraft(account, args.join(' ')); responseMessage = result; break;
        case 'daily': result = await handleDaily(account); responseMessage = `${username}, ${result.message}`; break;
        case 'flip': result = await handleFlip(account, parseInt(args[0]), args[1]); responseMessage = `${username}, ${result.message}`; break;
        case 'slots': result = await handleSlots(account, parseInt(args[0])); responseMessage = `${username}, ${result.message}`; break;
        case 'lb': case 'leaderboard': result = await handleLeaderboard(); responseMessage = result; break;
        case 'm': case 'market': const listings = await marketCollection.find().limit(20).toArray(); if (listings.length === 0) { responseMessage = 'The market is empty.' } else { responseMessage = listings.map((l, i) => `${i + 1}. (ID: ${l._id.toString().slice(-6)}) ${l.quantity}x ${ITEMS[l.itemId].name} @ ${l.price} ea. by ${l.sellerName}`); responseMessage.unshift('**Market Listings:**'); } break;
        case 'ms': case 'marketsell': const itemName = args[0]; const quantity = parseInt(args[1]); const price = parseFloat(args[2]); const itemId = getItemIdByName(itemName); if(!itemId || !quantity || !price) { responseMessage = "Usage: !ms <item_name> <quantity> <price>"; } else if ((account.inventory[itemId] || 0) < quantity) { responseMessage = "You don't have enough of that item."; } else { await modifyInventory(account._id, itemId, -quantity); const listing = await marketCollection.insertOne({ sellerId: account._id, sellerName: account._id, itemId, quantity, price }); responseMessage = `Listed ${quantity}x ${ITEMS[itemId].name}. ID: ${listing.insertedId.toString().slice(-6)}`; } break;
        case 'mb': case 'marketbuy': const listingId = args[0]; let listingToBuy; try { const listingsArray = await marketCollection.find({}).toArray(); listingToBuy = listingsArray.find(l => l._id.toString().endsWith(listingId)); if (!listingToBuy) throw new Error(); } catch (e) { responseMessage = 'Invalid listing ID.'; break; } if(listingToBuy.sellerId === account._id) { responseMessage = "You can't buy your own listing."; break; } const totalCost = listingToBuy.quantity * listingToBuy.price; if(account.balance < totalCost) { responseMessage = `You can't afford this.`; break; } await updateAccount(account._id, { balance: account.balance - totalCost }); await modifyInventory(account._id, listingToBuy.itemId, listingToBuy.quantity); const sellerAccount = await getAccount(listingToBuy.sellerId); if (sellerAccount) await updateAccount(sellerAccount._id, { balance: sellerAccount.balance + (totalCost * (1 - MARKET_TAX_RATE)) }); await marketCollection.deleteOne({ _id: listingToBuy._id }); responseMessage = `You bought ${listingToBuy.quantity}x ${ITEMS[listingToBuy.itemId].name}!`; break;
        case 'mc': case 'marketcancel': const cancelId = args[0]; let listingToCancel; try { const listingsArray = await marketCollection.find({}).toArray(); listingToCancel = listings let listingToBuy; try { const listingsArray = await marketCollection.find({}).toArray(); listingToBuy = listingsArray.find(l => l._id.toString().endsWith(listingId)); if (!listingToBuy) throw new Error(); } catch (e) { return interaction.editReply({ content: 'Invalid listing ID.' }); } if (listingToBuy.sellerId === account._id) return interaction.editReply({ content: "You can't buy your own listing." }); const totalCost = listingToBuy.quantity * listingToBuy.price; if (account.balance < totalCost) return interaction.editReply({ content: `You can't afford this. It costs ${totalCost} ${CURRENCY_NAME}.` }); await updateAccount(account._id, { balance: account.balance - totalCost }); await modifyInventory(account._id, listingToBuy.itemId, listingToBuy.quantity); const sellerAccount = await getAccount(listingToBuy.sellerId); if (sellerAccount) await updateAccount(sellerAccount._id, { balance: sellerAccount.balance + (totalCost * (1 - MARKET_TAX_RATE)) }); await marketCollection.deleteOne({ _id: listingToBuy._id }); await interaction.editReply({ content: `You bought ${listingToBuy.quantity}x ${ITEMS[listingToBuy.itemId].name}!` }); break;
        case 'marketcancel': const cancelId = options.getString('listing_id'); let listingToCancel; try { const listingsArray = await marketCollection.find({}).toArray(); listingToCancel = listingsArray.find(l => l._id.toString().endsWith(cancelId)); if (!listingToCancel) throw new Error(); } catch (e) { return interaction.editReply({ content: 'Invalid listing ID.' }); } if (listingToCancel.sellerId !== account._id) return interaction.editReply({ content: 'This is not your listing.' }); await modifyInventory(account._id, listingToCancel.itemId, listingToCancel.quantity); await marketCollection.deleteOne({ _id: listingToCancel._id }); await interaction.editReply({ content: `You cancelled your listing for ${listingToCancel.quantity}x ${ITEMS[listingToCancel.itemId].name}.` }); break;
    }
}
...
app.post('/command', async (req, res) => {
    ...
    switch (command) {
        ...
        case 'ms': case 'marketsell': if (args.length < 3) { responseMessage = "Usage: !marketsell <item name> <qty> <price>"; } else { const itemName = args.slice(0, -2).join(' '); const qty = parseInt(args[args.length - 2]); const price = parseFloat(args[args.length - 1]); const itemId = getItemIdByName(itemName); if (!itemId || qty <= 0 || price <= 0) { responseMessage = "Invalid format."; } else if ((account.inventory[itemId] || 0) < qty) { responseMessage = "You don't have enough to sell."; } else { await modifyInventory(account._id, itemId, -qty); const listing = await marketCollection.insertOne({ sellerId: account._id, sellerName: account._id, itemId, quantity: qty, price }); responseMessage = `Listed ${qty}x ${ITEMS[itemId].name}. Listing ID: ${listing.insertedId.toString().slice(-6)}`; } } break;
