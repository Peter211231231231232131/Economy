...
const ITEMS = {
    'iron_ore': { name: "Iron Ore", emoji: "🔩" }, 'copper_ore': { name: "Copper Ore", emoji: "🟤" }, 'wood': { name: "Wood", emoji: "🪵" }, 'stone': { name: "Stone", emoji: "🪨" }, 'coal': { name: "Coal", emoji: "⚫" }, 'raw_crystal':{ name: "Raw Crystal", emoji: "💎" },
    'iron_ingot': { name: "Iron Ingot", emoji: "⛓️" }, 'copper_ingot':{ name: "Copper Ingot", emoji: "🧡" },
...
async function handleMarket(page = 1) { const listingsPerPage = 5; const totalListings = await marketCollection.countDocuments(); if (totalListings === 0) return ["The market is empty."]; const totalPages = Math.ceil(totalListings / listingsPerPage); page = Math.max(1, Math.min(page, totalPages)); const listings = await marketCollection.find().sort({ _id: -1 }).skip((page - 1) * listingsPerPage).limit(listingsPerPage).toArray(); const marketHeader = `**🏪 Market Listings (Page ${page}/${totalPages}) 🏪**`; const marketLines = listings.map(l => `> (ID: \`${l._id.toString().slice(-6)}\`) ${ITEMS[l.itemId]?.emoji || '📦'} **${l.quantity}x** ${ITEMS[l.itemId].name} @ **${l.price}** ${CURRENCY_NAME} ea. by *${l.sellerName}*`); return [marketHeader, ...marketLines]; }
...
client.on('interactionCreate', async (interaction) => {
    ...
    try { await handleSlashCommand(interaction); } 
...
async function handleSlashCommand(interaction) {
    ...
    switch (commandName) {
        ...
        case 'market': const page = options.getInteger('page') || 1; result = await handleMarket(page); await interaction.editReply({ content: result.join('\n') }); break;
...
app.post('/command', async (req, res) => {
    ...
    let result;
    // A function to strip Discord markdown for in-game display
    const cleanText = (text) => text.replace(/\*|\>|`|🎒|📜|/g, '').replace(/<a?:.+?:\d+>/g, '');
    ...
    switch (command) {
        ...
        case 'recipes': responseMessage = cleanText(handleRecipes()); break;
        case 'craft': 
        ...
        case 'lb': case 'leaderboard': result = await handleLeaderboard(); responseMessage = result.map(line => cleanText(line)); break;
        ...
        case 'm': case 'market': const pageArg = parseInt(args[0]); const page = !isNaN(pageArg) && pageArg > 0 ? pageArg : 1; result = await handleMarket(page); responseMessage = result.map(line => cleanText(line)); break;
