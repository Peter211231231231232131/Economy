...
const ITEMS = { 
    ...
    'raw_meat': { name: "Raw Meat", emoji: "🍖", type: "food", buff: { duration_ms: 1 * 60 * 1000, effects: { work_bonus_percent: -0.10 } } },
...
const GATHER_TABLE = { 
    ...
    'raw_meat': { baseChance: 0.20, minQty: 1, maxQty: 1 },
    'spicy_pepper': { baseChance: 0.03, minQty: 1, maxQty: 1 },
};
...
// =========================================================================
// MODIFIED: This function can now be used for any identifier (Drednot name OR Discord ID)
async function getAccount(identifier) {
    const idStr = String(identifier).toLowerCase();
    // This finds a fully linked account OR a temporary account (where _id IS the identifier)
    return await economyCollection.findOne({ $or: [{ _id: idStr }, { discordId: String(identifier) }] });
}

// MODIFIED: A more flexible account creation function for temporary/partial accounts
async function createNewAccount(identifier, type = 'drednot') {
    const idStr = String(identifier).toLowerCase();
    const newAccount = {
        _id: idStr, // For Discord-only accounts, this will be the Discord ID. For Drednot-only, the name.
...
// MODIFIED: Can now update based on any unique ID (_id or discordId)
...
async function handleEat(account, foodName) { const foodId = getItemIdByName(foodName); if (!foodId) return `Could not find a food named "${foodName}".`; const itemDef = ITEMS[foodId]; if (itemDef.type !== 'food') return `You can't eat a ${itemDef.name}!`; if (!account.inventory[foodId] || account.inventory[foodId] < 1) return `You don't have any ${itemDef.name} in your inventory.`; await modifyInventory(account._id, foodId, -1); const now = Date.now(); const buff = { itemId: foodId, expiresAt: now + itemDef.buff.duration_ms }; await economyCollection.updateOne({ _id: account._id }, { $push: { activeBuffs: buff } }); let effectDescriptions = []; if (itemDef.buff.effects) { if (itemDef.buff.effects.gather_cooldown_reduction_ms) effectDescriptions.push(`gather cooldown reduced by ${itemDef.buff.effects.gather_cooldown_reduction_ms / 1000}s`); if (itemDef.buff.effects.work_cooldown_reduction_ms) effectDescriptions.push(`work cooldown reduced by ${itemDef.buff.effects.work_cooldown_reduction_ms / 1000}s`); if (itemDef.buff.effects.work_bonus_percent) { const verb = itemDef.buff.effects.work_bonus_percent > 0 ? 'increased' : 'decreased'; effectDescriptions.push(`work earnings ${verb} by ${Math.abs(itemDef.buff.effects.work_bonus_percent * 100)}%`); } if (itemDef.buff.effects.work_double_or_nothing) effectDescriptions.push(`your work earnings are now double or nothing`); } const durationText = formatDuration(itemDef.buff.duration_ms / 1000); const effectsText = effectDescriptions.length > 0 ? `Your ${effectDescriptions.join(', ')}.` : ''; return `You eat the ${itemDef.name}. ${effectsText} This effect will last for ${durationText}!`; }

// NEW HELPER FUNCTION: The core logic for merging two accounts
async function handleAccountMerge(discordId, drednotName) {
    const drednotNameLower = drednotName.toLowerCase();

    // Find both the temporary Discord account and the Drednot account
    const discordAccount = await economyCollection.findOne({ _id: discordId });
    const drednotAccount = await economyCollection.findOne({ _id: drednotNameLower });

    // If one doesn't exist, we can't merge. Link normally.
    if (!discordAccount || !drednotAccount) {
        let updates = { discordId: discordId, drednotName: drednotName };
        await economyCollection.updateOne({ _id: drednotNameLower }, { $set: updates });
        return { success: true, message: `✅ Verification successful! Your accounts are now linked.` };
    }

    // CONFLICT RESOLUTION: Check for active smelting jobs
    if (discordAccount.smelting && drednotAccount.smelting) {
        return { success: false, message: "❌ Merge Failed: You cannot link while both your Discord and in-game accounts have active smelting/cooking jobs. Please wait for one to finish." };
    }
    ...
    const mergedData = {
        ...
        // 3. Combine Cooldowns (take the most recent)
        ...
        // 4. Combine Active States (prioritize the one that exists)
        ...
        // 5. Set final identifiers
        discordId: discordId,
        drednotName: drednotName
    };

    // Merge inventories carefully
...
async function handleSlashCommand(interaction) {
    ...
    if (commandName === 'link') {
        ...
        const existingDiscordAccount = await getAccount(user.id);

        // Check if fully linked already
        if (existingDiscordAccount && existingDiscordAccount.discordId === user.id && existingDiscordAccount.drednotName) {
            return interaction.editReply({ content: `Your Discord account is already linked to the Drednot account **${existingDiscordAccount.drednotName}**.` });
        }

        // Check if target Drednot account is taken
        ...
        let replyMessage = `**Verification Started!**\nIn Drednot, type: \`!verify ${verificationCode}\`\nThis code expires in 5 minutes.`;
        
        // MODIFIED: Add a warning if they have temporary progress
        if (existingDiscordAccount) {
            replyMessage += `\n\n**Note:** You have temporary progress on your Discord account. Verifying will merge this progress with your **${drednotNameToLink}** account.`
        }
    ...
    // List commands that don't require an account
    if (['market', 'leaderboard', 'recipes', 'crateshop', 'iteminfo'].includes(commandName)) {
    ...
    // MODIFIED: Centralized account check and creation
    let account = await getAccount(user.id);
    if (!account) {
        // Create a temporary "Discord-only" account and let them proceed
        account = await createNewAccount(user.id, 'discord');
        // Let the command run with the newly created account
    }

    // From here, all commands can assume `account` exists.
...
app.post('/command', async (req, res) => {
    ...
    if (command === 'verify') {
        ...
        if (!verificationData || (Date.now() - verificationData.timestamp > 5 * 60 * 1000)) {
            ...
            // MODIFIED: This now calls the powerful merge function
            ...
            if (mergeResult.success) {
                 ...
                 try {
                    ...
                    discordUser.send(mergeResult.message);
                } catch (e) { console.log("Couldn't send DM confirmation for merge."); }
            }
        }
...
async function startServer() {
    ...
    setInterval(processFinishedSmelting, 5000); // Check every 5 seconds
}
