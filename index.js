...
async function handleSlashCommand(interaction) {
    ...
    if (commandName === 'link') {
        ...
        await verificationsCollection.insertOne({ _id: code, discordId: user.id, drednotName: drednotNameToLink, timestamp: Date.now() });
        await interaction.editReply({ content: `**Verification Started!**\nIn Drednot, type: \`!verify ${code}\`\nThis code expires in 5 minutes.` });
        return;
    }
    ...
    switch (commandName) {
        case 'balance': await interaction.editReply({ content: `Your balance for **${account._id}** is: ${account.balance} ${CURRENCY_NAME}.` }); break;
