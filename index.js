...
client.on('interactionCreate', async (interaction) => { if (!interaction.isChatInputCommand()) return; try { await handleSlashCommand(interaction); } catch (error) { console.error("Error handling slash command:", error); if (interaction.replied || interaction.deferred) await interaction.editReply({ content: 'An unexpected error occurred!' }); }});
...
async function handleSlashCommand(interaction) {
    ...
    switch (commandName) {
        ...
        case 'leaderboard': lines = await handleLeaderboard(); title = "Leaderboard"; result = getPaginatedResponse(user.id, commandName, lines, title); await interaction.editReply({ content: [result.header, ...result.lines, result.footer].join('\n') }); break;
        ...
        case 'market': const page = options.getInteger('page') || 1; lines = await handleMarket(); title = "Market"; result = getPaginatedResponse(user.id, commandName, lines, title, page - 1); await interaction.editReply({ content: [result.header, ...result.lines, result.footer].join('\n') }); break;
...
app.post('/command', async (req, res) => {
    ...
    const cleanText = (text) => Array.isArray(text) ? text.map(t => t.replace(/\*\*|`|>/g, '')) : text.replace(/\*\*|`|>/g, '');
