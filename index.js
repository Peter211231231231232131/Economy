...
const ITEMS = { 
    ...
    // --- NEW FOOD ITEMS ---
    'wild_berries': { name: "Wild Berries", emoji: "🫐", type: "food", buff: { duration_ms: 5 * 60 * 1000, effects: { gather_cooldown_reduction_ms: 10 * 1000 } } }, // 5 mins, -10s gather cooldown
    'foragers_mushroom': { name: "Forager's Mushroom", emoji: "🍄", type: "food", buff: { duration_ms: 10 * 60 * 1000, effects: { gather_cooldown_reduction_ms: 5 * 1000 } } }, // 10 mins, -5s gather cooldown
};
const GATHER_TABLE = { 
    ...
    // --- ADDED FOOD TO GATHER TABLE ---
    'wild_berries': { baseChance: 0.15, minQty: 1, maxQty: 1 }, // 15% chance to find
    'foragers_mushroom': { baseChance: 0.10, minQty: 1, maxQty: 1 }, // 10% chance to find
};
...
async function handleGather(account) {
    const now = Date.now();
    let currentCooldown = GATHER_COOLDOWN_MINUTES * 60 * 1000;

    // --- BUFF LOGIC ---
    let activeBuffs = account.activeBuffs || [];
    const validBuffs = activeBuffs.filter(buff => buff.expiresAt > now);
    let cooldownReduction = 0;
    
    if (validBuffs.length < activeBuffs.length) { // Cleanup expired buffs
        await updateAccount(account._id, { activeBuffs: validBuffs });
        account.activeBuffs = validBuffs;
    }

    for (const buff of validBuffs) {
        const itemDef = ITEMS[buff.itemId];
        if (itemDef?.buff?.effects?.gather_cooldown_reduction_ms) {
    ...
    let gatheredItems = [];
    let updates = {};
    const shuffledOres = Object.keys(GATHER_TABLE).sort(() => 0.5 - Math.random());
    for (const itemId of shuffledOres) {
        if (gatheredItems.length >= maxTypes) break;
        if (Math.random() < GATHER_TABLE[itemId].baseChance) {
            let qty = Math.floor(Math.random() * (GATHER_TABLE[itemId].maxQty - GATHER_TABLE[itemId].minQty + 1)) + GATHER_TABLE[itemId].minQty;
            for (let i = 0; i < basketCount; i++) if (Math.random() < 0.5) qty++;
            updates[`inventory.${itemId}`] = qty;
            gatheredItems.push(`${ITEMS[itemId].emoji} ${qty}x ${ITEMS[itemId].name}`);
        }
    }
    await economyCollection.updateOne({ _id: account._id }, { $inc: updates, $set: { lastGather: now } });
    if (gatheredItems.length === 0) return { success: true, message: 'You searched but found nothing of value.' };
...
// --- NEW FUNCTION for eating food ---
async function handleEat(account, foodName) {
    const foodId = getItemIdByName(foodName);
    if (!foodId) {
        return `Could not find a food named "${foodName}".`;
    }

    const itemDef = ITEMS[foodId];
    if (itemDef.type !== 'food') {
        return `You can't eat a ${itemDef.name}!`;
    }

    if (!account.inventory[foodId] || account.inventory[foodId] < 1) {
        return `You don't have any ${itemDef.name} in your inventory.`;
    }
    ...
    const buff = {
        itemId: foodId,
        expiresAt: now + itemDef.buff.duration_ms
    };
    ...
    return `You eat the ${itemDef.name}. You feel its effects for the next ${durationText}!`;
}
...
async function handleSlashCommand(interaction) {
    ...
    if (commandName === 'iteminfo') { const itemName = options.getString('item_name'); const infoMessage = handleItemInfo(itemName); return interaction.editReply({ content: infoMessage }); }
...
app.post('/command', async (req, res) => {
    ...
    if (!account) { account = await createNewAccount(username); const welcomeMessage = [`Your new account has been created with ${STARTING_BALANCE} Bits.`, `Join the Discord for updates, support, and commands!`, `https://discord.gg/SvZe9ytB`, `NOTE: Use !link in-game to verify with your Discord account.` ]; return res.json({ reply: welcomeMessage }); }
    ...
    switch (command) {
        case 'iif': case 'iteminfo': if (args.length === 0) { responseMessage = "Usage: !iteminfo <item name>"; break; } const infoItemName = args.join(' '); const infoMessage = handleItemInfo(infoItemName); responseMessage = cleanText(infoMessage).split('\n'); break;
        case 'eat': if (args.length === 0) { responseMessage = "Usage: !eat <food name>"; break; } const foodName = args.join(' '); responseMessage = await handleEat(account, foodName); break;
        ...
        case 'bal': case 'balance': responseMessage = `Your balance is: ${account.balance} ${CURRENCY_NAME}.`; break;
        ...
        case 'flip': if (args.length < 2) { responseMessage = "Usage: !flip <amount> <h/t>"; } else { result = await handleFlip(account, parseInt(args[0]), args[1].toLowerCase()); responseMessage = result.message; } break;
        case 'slots': if (args.length < 1) { responseMessage = "Usage: !slots <amount>"; } else { result = await handleSlots(account, parseInt(args[0])); responseMessage = result.message; } break;
