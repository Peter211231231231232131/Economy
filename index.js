...
const SMELT_COOLDOWN_SECONDS_PER_ORE = 30;
const SMELT_COAL_COST_PER_ORE = 1;
...
// --- BACKGROUND JOB: Process finished smelting ---
...
async function handleSlashCommand(interaction) {
    ...
    switch (commandName) {
        ...
        case 'market': const listings = await marketCollection.find().sort({ _id: -1 }).limit(20).toArray(); if (listings.length === 0) return interaction.editReply({ content: 'The market is empty.' }); const marketMessage = listings.map(l => `(ID: ${l._id.toString().slice(-6)}) **${l.quantity}x** ${ITEMS[l.itemId].name} @ **${l.price}** ${CURRENCY_NAME} ea. by *${l.sellerName}*`).join('\n'); await interaction.editReply({ content: `**Market Listings:**\n${marketMessage}` }); break;
...
async function startServer() {
    ...
    setInterval(processFinishedSmelting, 15 * 1000); // Check for finished smelts every 15 seconds
