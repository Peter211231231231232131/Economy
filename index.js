// --- Library Imports ---
const { Client, GatewayIntentBits, MessageFlags } = require('discord.js'); // Add MessageFlags
...
// ... (rest of the database and helper functions are the same)
const DATA_DIR = './data';
...
function loadData() { try { if (!fs.existsSync(DATA_DIR)) { fs.mkdirSync(DATA_DIR); } if (fs.existsSync(ECONOMY_STORAGE_FILE)) { economyData = JSON.parse(fs.readFileSync(ECONOMY_STORAGE_FILE)); console.log('Economy database loaded from file.'); } else { console.log('No economy database found, starting fresh.'); economyData = {}; } if (fs.existsSync(PENDING_VERIFICATIONS_FILE)) { pendingVerifications = JSON.parse(fs.readFileSync(PENDING_VERIFICATIONS_FILE)); console.log('Pending verifications loaded from file.'); } else { console.log('No pending verifications found, starting fresh.'); pendingVerifications = {}; } } catch (err) { console.error('Error loading data from file:', err); economyData = {}; pendingVerifications = {}; } }
async function saveData() { try { if (!fs.existsSync(DATA_DIR)) { fs.mkdirSync(DATA_DIR); } await fs.promises.writeFile(ECONOMY_STORAGE_FILE, JSON.stringify(economyData, null, 2)); await fs.promises.writeFile(PENDING_VERIFICATIONS_FILE, JSON.stringify(pendingVerifications, null, 2)); } catch (err) { console.error('Error saving data to file:', err); } }
function getAccountKey(identifier) { const identifierStr = String(identifier).toLowerCase(); for (const key in economyData) if (key.toLowerCase() === identifierStr) return key; for (const key in economyData) if (economyData[key].discordId === String(identifier)) return key; return null; }
async function createNewAccount(drednotName) { const lowerName = drednotName.toLowerCase(); economyData[lowerName] = { balance: STARTING_BALANCE, discordId: null, lastWork: null }; await saveData(); return economyData[lowerName]; }
async function handleWorkCommand(accountKey) { const account = economyData[accountKey]; if (!account) return { success: false, message: 'Account not found.' }; const now = Date.now(); const cooldown = WORK_COOLDOWN_MINUTES * 60 * 1000; if (account.lastWork && (now - account.lastWork) < cooldown) { const remaining = cooldown - (now - account.lastWork); return { success: false, message: `You are on cooldown. Please wait another ${Math.ceil(remaining / 60000)} minute(s).` }; } const earnings = Math.floor(Math.random() * (WORK_REWARD_MAX - WORK_REWARD_MIN + 1)) + WORK_REWARD_MIN; account.balance += earnings; account.lastWork = now; await saveData(); return { success: true, message: `You worked hard and earned ${earnings} ${CURRENCY_NAME}! Your new balance is ${account.balance}.` }; }

// =========================================================================
// --- Discord Bot Logic (Updated for Modern Ephemeral Replies) ---
// =========================================================================
client.on('ready', () => {
...
client.on('interactionCreate', async (interaction) => {
  ...
  // --- UPDATED: Use flags for ephemeral deferral ---
  ...
  if (commandName === 'link') {
    if (accountKey) {
        // No change needed here, editReply doesn't use flags
