...
async function handleButtonInteraction(interaction) {
    ...
    if (!session) return interaction.update({ content: 'This list has expired or is invalid.', components: [] });
...
async function handleSlashCommand(interaction) {
    ...
    if (['market', 'leaderboard', 'recipes'].includes(commandName)) {
        ...
        const { discord } = getPaginatedResponse(user.id, type, lines, title, page - 1);
...
app.post('/command', async (req, res) => {
    ...
    const cleanText = (text) => Array.isArray(text) ? text.map(t => t.replace(/\*\*|`|>/g, '').replace(/<a?:.+?:\d+>/g, '')) : String(text).replace(/\*\*|`|>/g, '').replace(/<a?:.+?:\d+>/g, '');
